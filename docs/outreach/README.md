...
**A fifth question has been added to the thread's queue, 2026-08-25:** does the [12-month retention policy](https://discuss.px4.io/t/px4-flight-review-data-retention/41906) delete the `.ulg` while keeping the metadata record? The dump still describes logs from 2016, and only 35.7 % of our 79,477-log frame falls inside a 365-day window — so the answer is worth roughly 51,000 candidate flights. It cannot be settled by probing `download_url`, because that is precisely the automated retrieval `robots.txt` disallows and [ADR-0005](../adr/0005-sample-from-metadata-not-bulk-download.md) gates. Audit row A8.
...
| **The DPIA itself** — G1's remaining blocker, and the one in front of the first download | It is the controller's assessment and carries your judgement on residual risk; nobody else can sign it | The screening is done and says it is required ([`../08-dpia-screening.md`](../08-dpia-screening.md), [ADR-0011](../adr/0011-the-dpia-is-a-precondition-of-retrieval.md)). Art. 35(7)'s four elements are mapped to the documents that already answer them; what is genuinely missing is short — a written legitimate-interest statement, a why-no-less-intrusive-design argument, risk severity and likelihood grading, and the exclusion mechanism built rather than promised |
| Publishing the privacy notice | It carries your name as controller and your contact address | [`PRIVACY.md`](../../PRIVACY.md) and a forum-ready post are written; both have placeholders where the contact address goes |
...
| Asking A8 on the M0 thread | Outward-facing, and it is a reply to a thread under your name | The question is drafted in audit row A8 with the number that makes it worth asking — 51,075 frame logs sit outside the retention window. One paragraph, same thread |
